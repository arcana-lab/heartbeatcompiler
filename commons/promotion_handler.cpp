...
bool heartbeat_polling(task_memory_t *tmem) {
  ...
  return false;
#elif defined(PROMOTION_INSERTION_OVERHEAD_ANALYSIS)
  return true;
#endif
...
/*
 * Function invoked inside the promotion_handler (when received a heartbeat),
...
#if defined(POLLING_OVERHEAD_ANALYSIS)
#if defined(ENABLE_SOFTWARE_POLLING) && defined(CHUNK_LOOP_ITERATIONS) && defined(ADAPTIVE_CHUNKSIZE_CONTROL)
