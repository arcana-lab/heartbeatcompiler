...
cd /nfs-scratch/yso0488/heartbeatcompiler0/manual_transformation/benchmarks/floyd_warshall ;
mkdir -p ${results} ;
...
folder="${results}/${metric}/${bench}/${size}/heartbeat_manual_software_polling_${chunksize_0}_${chunksize_1}" ;
mkdir -p ${folder} ;
run_parallel "heartbeat" ${folder} ;

make clean ; make heartbeat ENABLE_ROLLFORWARD=true INPUT_CLASS=${class} INPUT_SIZE=${size} CHUNKSIZE_0=${chunksize_0} CHUNKSIZE_1=${chunksize_1} ;
folder="${results}/${metric}/${bench}/${size}/heartbeat_manual_rollforward_${chunksize_0}_${chunksize_1}" ;
mkdir -p ${folder} ;
run_parallel "heartbeat" ${folder} ;
