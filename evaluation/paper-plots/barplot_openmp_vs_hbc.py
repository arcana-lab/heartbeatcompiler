...
for i, s in enumerate(openmp_best_speedups):
    if math.isnan(openmp_best_stdev[i]):
        ...
        continue
    spaces = round(openmp_best_stdev[i] / 0.35) * ' '
...
fig = go.Figure(data=[
    go.Bar(name='OpenMP (best scheduling)', y=benchmarks, x=openmp_best_speedups, error_x=dict(type='data', array=openmp_best_stdev), marker_color=colors[4], text=openmp_text, textposition="outside", textfont_size=11, orientation='h'),
    go.Bar(name='Heartbeat Compiler', y=benchmarks, x=hbc_speedups, error_x=dict(type='data', array=hbc_stdev), marker_color=colors[2], text=hbc_text, textposition="outside", textfont_size=11, orientation='h')
], layout_xaxis_range=[0,69])

# Add shading
fig.add_hrect(y0=-0.5, y1=4.5, 
              annotation_text="Irregular workloads", annotation_font_size=13, annotation_y=2.5, annotation_yanchor="middle", annotation_x=0.735, annotation_xanchor="center",
              fillcolor="LightGrey", opacity=0.8, line_width=0, layer='below')
fig.add_hrect(y0=4.5, y1=8.5, 
              annotation_text="Regular workloads", annotation_font_size=13, annotation_y=6.5, annotation_yanchor="middle", annotation_x=0.74, annotation_xanchor="center",
