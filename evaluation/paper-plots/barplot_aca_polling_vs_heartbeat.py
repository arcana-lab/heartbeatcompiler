...
bar_colors=[colors[0],] * (len(chunksizes[1]) - 1)
bar_colors.append(colors[2])
marker_colors=[colors[4],] * (len(chunksizes[1]) - 1)
...
fig.add_annotation(text=str(wasted_polls[-1]), font_size=15, xref='paper', x=0.885, yref='paper', y=0.46, ayref='y domain', ay=-25)
fig.add_annotation(text="Static Chunksizes (log2)", font_size=20, showarrow=False, xref='paper', x=0.27, yref='paper', y=-0.15)
fig.add_annotation(text="ACA", font_size=20, showarrow=False, xref='paper', x=0.9155, yref='paper', y=-0.15)
...
fig.update_yaxes(type='log', range=[2,7], titlefont_size=20, dtick=1, tickfont_size=15, showgrid=True, gridwidth=1, gridcolor='grey', showline=True, mirror=True, linewidth=1, linecolor='black', secondary_y=False)
