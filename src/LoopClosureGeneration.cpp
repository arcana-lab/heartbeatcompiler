...
void HeartbeatTransformation::generateLoopClosure(
  LoopContent *lc
) {
  ...
  auto shouldThisVariableBeSkipped = [&](uint32_t variableID, bool isLiveOut) -> bool {
    ...
    if (this->loopToSkippedLiveIns[lc].find(le->getProducer(variableID)) != this->loopToSkippedLiveIns[lc].end()) {
      errs() << "skip " << *(le->getProducer(variableID)) << " of id " << variableID << " because of the skipped liveIns\n";
      return true;
    }
    ...
    if (producer->getNumUses() == 1 && (producer == startValue || producer == exitValue)) {
      errs() << "skip " << *(le->getProducer(variableID)) << " of id " << variableID << " because of initial/exit condition of the callee loop\n";
      return true;
    }
    // if a variable is a constant live-in variable (loop invariant), skip it
    auto envProducer = le->getProducer(variableID);
    if (this->loopToConstantLiveIns[lc].find(envProducer) != this->loopToConstantLiveIns[lc].end()) {
  ...
  this->initializeEnvironmentBuilder(lc, isReducible, shouldThisVariableBeSkipped, isConstantLiveInVariable);
   ...
   */
  auto headerClone = this->lsTask->getCloneOfOriginalBasicBlock(ls->getHeader());
  ...
  if (this->verbose > HBTVerbosity::Disabled) {
    errs() << this->outputPrefix << "loop-slice task after generating loop closure\n";
...
void HeartbeatTransformation::initializeLoopEnvironmentUsers() {
  for (auto i = 0; i < this->tasks.size(); ++i) {
    ...
    auto myLevelIndexV = cast<Value>(ConstantInt::get(int64, this->lna->getLoopLevel(this->lc) * 8));
    ...
    if (envBuilder->getSingleEnvironmentSize() > 0) {
      auto liveInEnvPtrGEPInst = entryBuilder.CreateInBoundsGEP(
        ...
        ArrayRef<Value *>({
          zeroV,
          ConstantInt::get(entryBuilder.getInt64Ty(), this->lna->getLoopLevel(this->lc) * this->valuesInCacheLine + this->liveInEnvIndex)
        }),
        "liveInEnvPtr"
      );
    ...
    if (envBuilder->getReducibleEnvironmentSize() > 0) {
      auto liveOutEnvPtrGEPInst = entryBuilder.CreateInBoundsGEP(
        ...
        ArrayRef<Value *>({
          zeroV,
          ConstantInt::get(entryBuilder.getInt64Ty(), this->lna->getLoopLevel(this->lc) * this->valuesInCacheLine + this->liveOutEnvIndex)
        }),
        "liveOutEnv_addr"
      );
...
void HeartbeatTransformation::generateCodeToLoadLiveInVariables(LoopContent *lc, int taskIndex) {
...
void HeartbeatTransformation::setReducableVariablesToBeginAtIdentityValue(LoopContent *lc, int taskIndex) {
...
void HeartbeatTransformation::generateCodeToStoreLiveOutVariables(LoopContent *lc, int taskIndex) {
  ...
  for (auto envID : envUser->getEnvIDsOfLiveOutVars()) {
    ...
    if (isReduced) {
      envUser->createReducableEnvPtr(entryBuilder, envID, envType, this->numTaskInstances, ((LoopSliceTask *)task)->getTaskIndexArg());
    } else {
    ...
    for (auto producerClone : producerClones) {
      ...
      auto insertBBs = this->determineLatestPointsToInsertLiveOutStore(lc, taskIndex, producerClone, isReduced, *taskDS);
      ...
      for (auto BB : insertBBs) {
        auto producerValueToStore = this->fetchOrCreatePHIForIntermediateProducerValueOfReducibleLiveOutVariable(lc, taskIndex, envID, BB, *taskDS);
...
void HeartbeatTransformation::allocateNextLevelReducibleEnvironmentInsideTask(LoopContent *lc, int taskIndex) {
  auto loopStructure = lc->getLoopStructure();
