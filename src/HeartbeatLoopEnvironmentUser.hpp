...
public:
  ...
  inline Value *getLocationToStoreReductionArray() { return this->locationToStoreReductionArray; };
  ...
  inline void setLiveOutEnvAddress(Value *liveOutEnvBitcastInst) { this->liveOutEnvBitcastInst = liveOutEnvBitcastInst; };
  ...
  inline void resetReducibleEnvironmentArray(IRBuilder<> &builder) {
    ...
    if (this->reducibleEnvIDToIndex.size() > 1) {
      ...
      builder.CreateStore(
        this->reductionArrayForSingleLiveOut,
...
private:
  ...
  // if we have at least 2 live-outs, then this instruction is casted after loading from the context array
  // otherwise (only 1 live-out), this is the address in the context array, which reduction array will get loaded from
